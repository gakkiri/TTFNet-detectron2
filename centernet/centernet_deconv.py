...
class CenternetDeconv(nn.Module):
    ...
    def __init__(self, cfg, output_shape):
        ...
        self.shortcut1 = ShortcutConv2d(output_shape['res4'].channels, channels[1], 1)
        self.shortcut2 = ShortcutConv2d(output_shape['res3'].channels, channels[2], 2)
        self.shortcut3 = ShortcutConv2d(output_shape['res2'].channels, channels[3], 3)

    def forward(self, xs):
        # x = self.deconv1(x)
        # x = self.deconv2(x)
        # x = self.deconv3(x)
        x2, x3, x4, x5 = xs.values()
        x = self.deconv1(x5) + self.shortcut1(x4)
        x = self.deconv2(x) + self.shortcut2(x3)
        x = self.deconv3(x) + self.shortcut3(x2)
        return x
